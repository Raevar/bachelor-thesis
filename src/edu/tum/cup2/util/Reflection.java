...
public class Reflection
{
	 ...
	 */
	private static Terminal[] termcache;
	public static Terminal[] getTerminals()
	{
		if (termcache!=null) return termcache;
		try
		{
	 ...
	 * the current stack trace.
	 */
	private static NonTerminal[] nontermcache;
	public static NonTerminal[] getNonTerminals()
	{
		if (nontermcache!=null) return nontermcache;
		try
		{
			//get terminals as Objects
	...
	private static SymbolValueClasses svccache;
	/**
		...
		try
		{
			...
			for (int i = 0; i < childClasses.length; i++)
			{
				...
				if (SymbolValue.class.isAssignableFrom(childClass))
				{
					...
					String myclass = childClass.getSimpleName();
					Symbol symbol = getSymbolByName(myclass, terminals, nonTerminals);
					if (symbol == null)
					{
	...
	public static void checkParamsOfAction(Method method, int position,
		...
		for (int i = 0; i < parameters.length; i++)
		{
			...
			while (true)
			{
				if (!rhs.hasNext())
				{
					throw new IllegalSpecException("Parameters do not match for an action for the RHS: " + rhsSymbols.toString()+" parameter["+curPosition+"](expected: "+paramType+")");
				}
				Symbol rhsSymbol = rhs.next();
				curPosition++;
...
//      System.out.println(obj.getClass() + " : ( " + obj.name() + " , " + obj.ordinal()+" ) ");
  ...
  public static Object getFieldValueOrNull(Object obj, String field_name){
