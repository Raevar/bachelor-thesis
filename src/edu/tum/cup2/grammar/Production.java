...
public final class Production implements Serializable
{
	...
	private static HashMap<String, Object> specMap = new HashMap<String, Object>();
	@SuppressWarnings("unchecked")
	private void deserializeReduceAction(ObjectInputStream stream) throws IOException, ClassNotFoundException
	{
		try
		{
			...
			if (specMap.containsKey(clazz_enclosing.getName().intern())){
				outer = specMap.get(clazz_enclosing.getName().intern());
				System.out.println("Recovered a "+specMap.get(clazz_enclosing.getName()));
				System.out.println(" thread id"+Thread.currentThread().getId());
			}
			else {
				//System.out.println("could not find "+clazz_enclosing.getName()+" "+specMap.keySet().size());
			}
				...
				if (cParms.length == 1)
				{
					if (cParms[0].equals(clazz_enclosing))
					{
						...
						if (outer == null)
						{
							...
							specMap.put(clazz_enclosing.getName().intern(), outer);
							//System.out.println("Recreated a "+clazz_enclosing.getName()+" "+specMap.get(clazz_enclosing.getName()));
						}
